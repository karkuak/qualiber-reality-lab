...
import { coreHash, domainHash, HASH_DOMAINS, sealSigned, treeHash, type SigningKey } from "@erl2/integrity";
...
export class EnvironmentRun {
  ...
  provision(): { readonly inventory: EnvironmentResourceInventoryV1; readonly leases: number } {
    ...
    if (result.partial) {
      // A partial provision is a Lab-owned provisioning failure whose authorized
      // route is `case_selected -> invalid_failure_detected` and the environment
      // cleanup frontier. That invalid terminal is not part of this slice, so the
      // refusal is typed and loud rather than quietly continuing on half an
      // environment.
      throw new Erl2Error(
  ...
  restore(): EnvironmentRestorationVerificationV1 {
    ...
    if (!restoration.passed) {
      throw new Erl2Error(
        ...
        "restoration did not pass; the authorized route is receipt-backed emergency cleanup, which is not part of this slice",
        { owner: "lab" },
      );
    }
    void inventory;
  ...
  destroy(): { readonly teardown: TeardownVerificationV1; readonly residue: number } {
    ...
    if (!teardown.passed) {
      throw new Erl2Error(
        ...
        `teardown left ${String(remaining.size)} resource(s); the authorized route is receipt-backed emergency cleanup, which is not part of this slice`,
        { owner: "lab" },
      );
    }
