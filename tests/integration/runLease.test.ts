...
test("RUN-LEASE: a mutating command is refused while a live foreign lease is present", () => {
  ...
  writeFileSync(
    leasePath,
    JSON.stringify({ owner: "other-process", command: "acquire", acquired_at_ms: now, expires_at_ms: now + 100_000, pid: 99999 }),
  );
  ...
  writeFileSync(
    leasePath,
    JSON.stringify({ owner: "other-process", command: "acquire", acquired_at_ms: now - 400_000, expires_at_ms: now - 100_000, pid: 99999 }),
  );
