...
| `ADAPTER_PROTOCOL_RESPONSE_MISMATCH` | The adapter's response did not match what its manifest declared for the operation. This is a response-shape refusal. It is **not** what a manifest operation-*order* mismatch produces — for that, see `ADAPTER_CERTIFICATION_SCOPE_MISMATCH` in the row below. | Compare the failing operation's response against what the manifest declares for it. | Only after the manifest or adapter is corrected. | Yes. | Remove `--output-root`; see [Cleanup](#cleanup). | Adapter integration (manifest/adapter mismatch), not the Lab or the subject. | Terminal status is not `observed_complete`. |
| `ADAPTER_CERTIFICATION_SCOPE_MISMATCH` | The operation list the adapter negotiated does not exactly match the selected certified profile. `AdapterHost` compares the two **positionally**, so the usual cause is a manifest whose operation order differs from the handler-key order the adapter SDK negotiates. This is the load-bearing refusal, and it is the one retained in the record. On the console you may instead see `ADAPTER_LOCAL_OPERATION_ORDER_INVALID` with prerequisite wording such as `operation stop requires completed start` — a downstream consequence of the failed operation moving the run to its frozen cleanup suffix, not a separate cause. | Compare your manifest's `protocol_support[].operations` against your adapter's negotiated handler-key order, in order, position by position. See [the operator-surface note](trusted-local-observation-operator-surface.md#your-manifests-operation-order-must-match-your-adapters). | Only after the manifest or adapter is corrected. | Yes, when the run reaches cleanup; with some plans the command refuses before a record is written. | Remove `--output-root`; see [Cleanup](#cleanup). | Adapter integration (manifest/adapter mismatch), not the Lab or the subject. | Terminal status is not `observed_complete`. |
...
| Refusal | Cause | Operator action | Retry meaningful? | Record written? | Cleanup | Concerns | Claim consequence |
|---|---|---|---|---|---|---|---|
| `ADAPTER_LOCAL_CLEANUP_INCOMPLETE` | The retained record's `terminal_status` is not `observed_complete`, or some operation did not reach `completed`. Commonly a plan that declares `start` but no matching `stop`, which the reducer will never treat as discharged — but an operation whose record state is `ambiguous_not_replayed` reached this same refusal for an unrelated reason: the adapter could not satisfy the host's required-local, V2-only offer. The retained protocol code differs by adapter kind. **SDK-built adapter:** the retained code is `ADAPTER_PROTOCOL_VERSION_MISMATCH` — the SDK refuses before writing any negotiation frame, so the host receives none; commonly the adapter's `AdapterDefinition` does not truthfully declare `supportedProtocolVersions: ["subject-adapter/v2"]`. The SDK's own `ADAPTER_PROTOCOL_DOWNGRADE_REFUSED` diagnostic stays in the adapter process's stderr and never appears in the retained record. **Raw or SDK-bypassing adapter:** the retained code is `ADAPTER_PROTOCOL_DOWNGRADE_REFUSED` — the adapter itself wrote a negotiation frame naming a protocol version the host did not offer, or otherwise violating required-local negotiation. | Read the retained record at the path the refusal names, specifically each entry's `operation_records[].recovery_observation` (effectful operations) or `failure_code` (non-effectful ones) — not just the top-level response, which reports `ADAPTER_LOCAL_CLEANUP_INCOMPLETE` and masks the underlying protocol refusal — before assuming a missing `stop`. If the retained code is `ADAPTER_PROTOCOL_VERSION_MISMATCH`, declare the protocols the adapter actually supports — `supportedProtocolVersions: ["subject-adapter/v2"]` in its `AdapterDefinition` — and ensure it can process the negotiated V2 frames. If the retained code is `ADAPTER_PROTOCOL_DOWNGRADE_REFUSED`, the adapter bypasses the SDK and has no `AdapterDefinition` to change: repair its raw wire-level negotiation. If it is a missing cleanup operation, fix the plan for the next run. | Only after the plan or adapter defect is fixed; the same plan reproduces the same incomplete terminal. | **Yes** — the record is retained either way, and the command exits nonzero. | The adapter's own external side effects (if any) are the adapter's responsibility; the Lab's own bounded cleanup is always attempted regardless of how the loop ended. Remove `--output-root` once you are done reading the retained record. | Depends on which operation stalled — adapter integration, plan authoring, or (rarely) the Lab's own cleanup dispatch. | The run supports no completion claim; the retained record's `cleanup` field states exactly what was and was not discharged. |
